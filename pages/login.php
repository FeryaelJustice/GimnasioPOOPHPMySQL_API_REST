...
function loginHTML($username, $password, $usernameErr, $passErr)
{
    ...
    echo '</span>
        ...
        <a href="/projects/tasku5dawes/index.php?page=register"><strong>¿Aún no tienes una cuenta? Haz click aquí para registrarte.</strong></a>
    </div>
</div>';
}
...
if (isset($_SESSION['usuario'])) {
    header('Location: /projects/tasku5dawes/index.php?page=reservar');
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['signin'])) {
        $usuari = new Usuari();
        if ($usuari->checkUser($_POST["username"], $_POST["password"])->correctOperation) {
            $usuari = $usuari->getUsuari($_POST["username"], $_POST["password"])->data;
