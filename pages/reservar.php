...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['reservar'])) {
        ...
        if ($validFormVacios && !($reserva->checkSiExisteReserva($dataCompleta, $tipus)->correctOperation)) {
            if ($reserva->afegirReserva(new ReservaSingleInstance($dataCompleta, $tipus, $usuari_id))->correctOperation) {
                ...
                <select name="tipus" id="tipus" value="<?php echo $tipus ?>">
                    ...
                    for ($i = 0; $i < count($pistes); $i++) {
                        if ($pistes[$i]->id == $_POST["tipus"]) {
                            ...
                            echo '<option value="' . $pistes[$i]->id . '">' . $pistes[$i]->type . '</option>';
                        }
                    }
                    ?>
                </select>
